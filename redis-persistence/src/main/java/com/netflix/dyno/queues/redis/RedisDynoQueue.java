...
public class RedisDynoQueue implements DynoQueue {
    ...
    public RedisDynoQueue(Clock clock, String redisKeyPrefix, String queueName, Set<String> allShards, String shardName, int unackScheduleInMS, ShardingStrategy shardingStrategy) {
		...
		ThreadFactory threadFactory = new ThreadFactoryBuilder().setNameFormat("Thread-RedisDynoQueue-Scheduled-%d").build();

        schedulerForUnacksProcessing = Executors.newScheduledThreadPool(1,threadFactory);
    ...
    private int doPrefetchIdsHelper(String queueShardName, AtomicInteger prefetchCounter,
        ...
        try {
            ...
            // update by manheiba @ 20191219 for del repeat message
            ids.forEach(id -> {
            	if(!prefetchedIdQueue.contains(id)) {
            		prefetchedIdQueue.add(id);
    			}
            });
        ...
        try {
            ...
            return execute("remove", "(a shard in) " + queueName, () -> {
                ...
                for (String shard : allShards) {
                    ...
                    //update by manheiba @20191219 rollback to dyno-queues-2.0.0-rc5. for remove message
